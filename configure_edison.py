...
def verified(selection):
    verify = raw_input(
        "Is " + text_colors.MAGENTA + selection + text_colors.END + " correct? " + text_colors.YELLOW + "[Y or N]" + text_colors.END + ": ")
...
def _changeP2PSSID(newName):
    subprocess.call(["sed", "-i", "s/^p2p_ssid_postfix=.*/p2p_ssid_postfix=" + newName + "/",
...
def configureHiddenNetwork(ssid):
  ...
  while 1:
    ...
    if security == 0:
      ...
      password = ''
      while len(password) < 8 or len(password) > 50:
...
def configureNetworkAP(changewifi):
  ...
  if protocol == "OPEN":
    ...
    if len(password) != 5 and len(password) != 26 and len(password) != 13:
        print "Password must be either 5, 26 or 13 characters."
        return None
    if len(password) == 26:
       return wpa_templates.WEP_HEX % (ssid, "scan_ssid=1", password)
    else:
...
def removeFolderContents(folder_path):
    try:
        ...
        ret_value = subprocess.call("rm -rf " + folder_path + "/* " + folder_path + "/.[!.]* " + folder_path + "/..?*",
                                    shell=True)
        if ret_value != 0:
...
def getSSID():
    return subprocess.check_output("grep -o '^ssid=.*' /etc/hostapd/hostapd.conf | awk -F'=' '{print $2}'",
                                   shell=True).strip()
...
def disableOneTimeSetup(persist):
    ...
    if persist:
        print "disableOneTimeSetup done, incoming value - true"
...
def main():
    ...
    parser.add_argument('--restartWithAP', dest='restartwithap', help=argparse.SUPPRESS, action='store_true',
                        default=False)
    ...
    group_interactive.add_argument('--setup', dest='setup',
                                   help='Goes through changing the device name, password, and wifi options',
                                   action='store_true', default=False)
    group_interactive.add_argument('--name', dest='name', help='Changes the device name', action='store_true',
                                   default=False)
    group_interactive.add_argument('--password', dest='password', help='Changes the device password',
                                   action='store_true', default=False)
    group_interactive.add_argument('--wifi', dest='wifi', help='Changes the wifi options', action='store_true',
                                   default=False)

    group_non_interactive = root_group.add_mutually_exclusive_group()
    group_non_interactive.add_argument('--isRestartWithAPSet', dest='isrestartwithapset', help=argparse.SUPPRESS,
                                       action='store_true', default=False)
    group_non_interactive.add_argument('--showWiFiIP', dest='showwifiip',
                                       help='IP address associated with the wireless interface', action='store_true',
                                       default=False)
    group_non_interactive.add_argument('--showWiFiMode', dest='showwifimode',
                                       help='Show current mode for the wireless interface', action='store_true',
                                       default=False)
    group_non_interactive.add_argument('--version', dest='version', help='Gets the current firmware version',
                                       action='store_true', default=False)
    group_non_interactive.add_argument('--latest-version', dest='latestversion',
                                       help='Gets the latest firmware version', action='store_true', default=False)
    group_non_interactive.add_argument('--disableOneTimeSetup', dest='disableonetimesetup', help='Disable one-time setup with WiFi access point and enable WiFi client mode \
                                       Append --persist to retain this setting after reboot', action='store_true',
                                       default=False)
    ...
    group_non_interactive.add_argument('--toggleOneTimeSetup', dest='toggleonetimesetup', help='Switch between one-time setup with WiFi access point and WiFi client mode, and visa-versa. \
                                       Append --persist to retain this setting after reboot', action='store_true',
                                       default=False)
    group_non_interactive.add_argument('--upgrade', dest='upgrade',
                                       help='Downloads the latest firmware. Append --restartWithAP to reboot in WiFi access point mode after flashing', \
                                       action='store_true', default=False)
    group_non_interactive.add_argument('--flash', metavar=('<version>', '<release name>'), nargs='+', dest='otaflash',
                                       help='Downloads and flashes an image \
    ...
    group_non_interactive.add_argument('--changePassword', metavar='password', dest='changepassword', const='',
                                       help=argparse.SUPPRESS, nargs='?')
    group_non_interactive.add_argument('--changeName', metavar='name', dest='changename', help=argparse.SUPPRESS,
                                       nargs=1)
    group_non_interactive.add_argument('--changeWiFi', metavar='securityType SSID [Identity | password]',
                                       dest='changewifi', help=argparse.SUPPRESS, nargs='+')
    group_non_interactive.add_argument('--showNames', dest='shownames', help='Show device name and SSID',
                                       action='store_true', default=False)
    group_non_interactive.add_argument('--scanWiFi', dest='scanwifi', help=argparse.SUPPRESS, default=False)
    group_non_interactive.add_argument('--isPasswordSetupDone', dest='ispasswordsetupdone', help=argparse.SUPPRESS,
                                       action='store_true', default=False)
    ...
    if args.scanwifi:
        (ssid_keys, network_map) = scanForNetworksResults()
        print "scan results:"
        for i in range(0, len(ssid_keys)):
            print "SSID={0}|SECURITY_TYPE={1}".format(ssid_keys[i], network_map[ssid_keys[i]])

    if args.ispasswordsetupdone:
        if os.path.isfile(STATE_DIR + "/password-setup.done"):
            print "password_setup_done=true"
        else:
            print "password_setup_done=false"
